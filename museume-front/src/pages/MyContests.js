...
import { getMyContests } from '../redux/slices/contestSlice';
...
const ContestList = () => {
  ...
  const { myContests, isLoading } = useSelector((state) => state.contest);
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState('');

  useEffect(() => {
    dispatch(getMyContests({ page: 1, search, filter }));
  }, [dispatch, search, filter]);

  const handleSeeMore = () => {
    if (myContests?.next) {
      setPage(prev => prev + 1);
      dispatch(getMyContests({ page: page + 1, search, filter }));
    }
  };
  ...
  const handleDetailsClick = (contestId) => {
    navigate('/contest-detail', { state: { contestId } });
  };
    ...
    <>
      ...
      <div className="contest-container">
        ...
        <div className="contest-section">
          ...
          {myContests?.results?.filter(contest => isContestActive(contest.end_date)).map(contest => (
            <div key={contest.id}>
